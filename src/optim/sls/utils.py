...
def check_goldstein_conditions(
    step_size, loss, grad_norm, loss_next, c, beta_b, beta_f, bound_step_size, eta_max
):
    found = 0
...
def reset_step(
    step_size, n_batches_per_epoch=None, gamma=None, reset_option=1, init_step_size=None
):
    if reset_option == 0:
        pass
...
def get_adam_grad_list(group, state):
    params = group["params"]
    beta1, beta2 = group["adam_betas"]
    eps = group['adam_eps']

    def _get_modified_grad(p):
        grad = p.grad

        exp_avg = state[p]['exp_avg']
        exp_avg_sq = state[p]['exp_avg_sq']
        step_t = state[p]['step_t']
